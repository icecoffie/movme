...
// Base URL API Jikan untuk mengambil data anime
const API_BASE = 'https://api.jikan.moe/v4';

// Key yang digunakan untuk menyimpan data ke localStorage
...
// JSON.parse mengubah string menjadi array/object.
// Jika belum ada data, gunakan array kosong.
let library = JSON.parse(localStorage.getItem(STORAGE_KEY)) || [];

// Filter aktif untuk halaman library
let currentFilter = 'All';


// ===============================================
// INIT SAAT WEBSITE SELESAI LOAD
// ===============================================

// DOMContentLoaded memastikan HTML sudah siap
document.addEventListener('DOMContentLoaded', () => {
    fetchTopAnime();        // Ambil data awal dari API
    updateLibraryCount();   // Update jumlah koleksi dari localStorage
});
...
async function fetchTopAnime() {
    try {
        ...
        // Ambil 1 anime teratas untuk Hero Section
        const res = await fetch(`${API_BASE}/top/anime?filter=airing&limit=1`);
        const data = await res.json();

        // Jika ada data, kirim ke setupHero
        if(data.data && data.data.length > 0) {
            setupHero(data.data[0]);
        }

        // Ambil grid default
        fetchByGenre(null, document.querySelector('.genre-btn'));
...
async function fetchByGenre(genreId, btnElement) {
    ...
    // Reset semua tombol genre ke style default
    ...
    try {
        ...
        // Tentukan URL API berdasarkan genre
...
async function searchAnime(query) {
    ...
    try {
        ...
        // Jika sedang di library, pindah ke discover
...
// Event ketika tekan Enter di search input
...
function setupHero(anime) {
    ...
    document.getElementById('hero-play-btn').onclick = () => 
        openVideo(trailer, title);

    // Tombol add
    document.getElementById('hero-add-btn').onclick = () => 
        addToLibrary(anime.mal_id, title, img, anime.score);
}


// ===============================================
// RENDER GRID DISCOVER
// ===============================================
        ...
        card.innerHTML = `
            <div class="relative w-full pt-[140%] bg-gray-900">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300 gap-3 p-4">
                    ...
                    ${isSaved 
                        ? `<span class="text-green-400 text-xs font-bold bg-black/50 px-3 py-1 rounded-full border border-green-500/30">
                            <i class="fa-solid fa-check"></i> Saved
                           </span>`
                        : `<button onclick="addToLibrary('${item.mal_id}', '${escapeHtml(title)}', '${img}', '${score}')" 
                            class="text-white text-xs font-bold border border-white/30 hover:bg-white hover:text-black px-4 py-2 rounded-full transition">
                            + Add to List
                           </button>`
                    }
                </div>
            ...
            <div class="p-3">
                <h4 class="text-gray-200 text-sm font-bold truncate hover:text-blue-400 transition cursor-pointer" title="${title}">
                    ${title}
                </h4>
                <div class="flex items-center justify-between mt-1 text-[11px] text-gray-500">
                    <span>${item.year || 'Unknown'}</span>
                    <span class="border border-gray-700 px-1 rounded text-[9px]">
                        ${item.type || 'TV'}
                    </span>
                </div>
            </div>
        ...
        card.innerHTML = `
            <div class="relative w-full pt-[140%]">
                ...
                <button onclick="removeFromLibrary('${item.id}')" 
                    class="absolute top-2 right-2 w-6 h-6 rounded-full bg-black/60 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-red-600 transition z-10">
                    <i class="fa-solid fa-minus text-xs"></i>
                </button>
            </div>
            <div class="p-3">
                <h4 class="text-white text-sm font-bold truncate mb-2">
                    ${item.title}
                </h4>
                <select onchange="updateStatus('${item.id}', this.value)" 
                    class="w-full bg-[#0b0c15] text-xs text-gray-400 border border-gray-700 rounded p-1.5 focus:border-blue-500 outline-none">
